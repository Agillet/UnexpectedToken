...
function TextSet(ctxt, font, colour, strings, padx, pady, shadowColour,
  ...
  shadowOffsets.length && (ctxt.shadowOffsetY = shadowOffsets[1]);
...
function AddShadowToImage(i, w, h, scale, sc, sb, so) {
  ...
  so && (c.shadowOffsetX = so[0] * scale);
  so && (c.shadowOffsetY = so[1] * scale);
...
Tproto.Clicked = function (e) {
  ...
  if (t !== '' && t !== '_self') {
    if (this.frames[t]) {
      this.frames[t].document.location = h;
    } else {
      // try {
      //   if (top.frames[t]) {
